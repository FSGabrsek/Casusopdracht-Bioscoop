...
test('should send an sms when executing a timed reminder on a saved order with an sms alert', () => { 
    const order = new Order(1, new StandardPriceStrategy(), new PlainTextExportStrategy());
    ...
    order.timedOperation();

    expect(sendMessageSpy).toBeCalled();
});

test('should send an email when executing a timed reminder on a saved order with an email alert', () => { 
    const order = new Order(1, new StandardPriceStrategy(), new PlainTextExportStrategy());
...
test('should send a whatsapp messaege when executing a timed reminder on a saved order with a whatsapp alert', () => { 
    const order = new Order(1, new StandardPriceStrategy(), new PlainTextExportStrategy());
...
test('should send an sms when executing a timed reminder on a provisional order with an sms alert', () => { 
    const order = new Order(1, new StandardPriceStrategy(), new PlainTextExportStrategy());
    const service = new SmsService();
    const manager = new AlertManager(service);
    const sendMessageSpy = jest.spyOn(service, 'sendMessage');
...
test('should send an email when executing a timed reminder on a provisional order with an email alert', () => { 
    const order = new Order(1, new StandardPriceStrategy(), new PlainTextExportStrategy());
    const service = new EmailService();
    const manager = new AlertManager(service);
    const sendMessageSpy = jest.spyOn(service, 'sendMessage');
...
test('should send a whatsapp messaege when executing a timed reminder on a provisional order with a whatsapp alert', () => { 
    const order = new Order(1, new StandardPriceStrategy(), new PlainTextExportStrategy());
    const service = new WhatsappService();
    const manager = new AlertManager(service);
    const sendMessageSpy = jest.spyOn(service, 'sendMessage');
