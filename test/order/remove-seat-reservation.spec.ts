...
test('should not throw a state exception when removing a reservation from an open order', () => {
    ...
    expect(() => order.removeSeatReservation(ticket)).not.toThrowError(IllegalStateException);
});

test('should not throw a state exception when removing a reservation from a saved order', () => {
    const movie = new Movie('movie title');
    const screening = new MovieScreening(movie, new Date(), 10);
    const ticket =  new MovieTicket(screening, false, 1, 1);
    const order = new Order(1, new StandardPriceStrategy(), new PlainTextExportStrategy());
...
test('should throw a state exception when removing a reservation from a provisional order', () => {
    const movie = new Movie('movie title');
    const screening = new MovieScreening(movie, new Date(), 10);
    const ticket =  new MovieTicket(screening, false, 1, 1);
    const order = new Order(1, new StandardPriceStrategy(), new PlainTextExportStrategy());
    ...
    expect(() => order.removeSeatReservation(ticket)).toThrowError(IllegalStateException);
});

test('should throw a state exception when removing a reservation from a processed order', () => {
    const movie = new Movie('movie title');
    const screening = new MovieScreening(movie, new Date(), 10);
    const ticket =  new MovieTicket(screening, false, 1, 1);
    const order = new Order(1, new StandardPriceStrategy(), new PlainTextExportStrategy());
